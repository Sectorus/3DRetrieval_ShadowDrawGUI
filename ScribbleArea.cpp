...
void ScribbleArea::updateReferences() {
    ...
    cv::resize(result, result, cv::Size(m.rows, m.cols), 0, 0, cv::INTER_CUBIC);
    ...
    if (refs.size() > 0) {
        ...
        cv::Mat finished = blend.alphaBlend(blended, 0.5, conv_loaded, 0.5);
...
void ScribbleArea::setThreshold() {
    ...
    double val = QInputDialog::getDouble(this, tr("QInputDialog::getDouble()"),
                                         tr("Similarity Threshold (Standard 0.90):"),
                                         ResourceManager::instance()->similarity_threshold_, 0.00, 1.00, 2, &ok);
    if (ok) {
...
void ScribbleArea::eraseLine(const QPointF &endPointF) {
    ...
    for (int i = 0; i < lastP; i++) {
        if (eraseRect->contains(points[i].first.x() * nowFactor, points[i].first.y() * nowFactor) ||
            eraseRect->contains(points[i].second.x() * nowFactor, points[i].second.y() * nowFactor)) {
